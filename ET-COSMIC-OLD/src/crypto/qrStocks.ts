 ...
 */
export function collapseMeasurement(
  stock: QRStock,
  material?: Uint8Array,
): MeasurementResult {
  const mat = material ?? offlineMaterialFromSeed(`qr:${stock.symbol}`, 32);
  ...
  let rand = unit(mat, 0) * totalPriceProb;
  ...
  rand = unit(mat, 1) * totalVolumeProb;
...
export class QRMarket {
   ...
   */
  measureStock(symbol: string, material?: Uint8Array): MeasurementResult {
    const stock = this.stocks.get(symbol);
    if (!stock) throw new Error(`Ação ${symbol} não encontrada`);

    const result = collapseMeasurement(stock, material);
