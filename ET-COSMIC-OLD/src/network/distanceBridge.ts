...
import { BLE_MAX_AD_BYTES } from "../protocol/amp/knownLimitations";
...
export class DistanceBridge {
  ...
  private async tryChannel(channel: DistanceBridgeChannel, shard: Shard): Promise<boolean> {
    if (channel === "BLE") {
      ...
      const payload =
        shard.data.length > BLE_MAX_AD_BYTES
          ? shard.data.slice(0, BLE_MAX_AD_BYTES)
          : shard.data;
      await this.deps.ble.startAdvertising(payload);
      return true;
    }
