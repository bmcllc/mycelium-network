...
import { deriveHexId, offlineMaterialFromSeed, unit } from "../lib/moduleRealityBackend";
...
export class JanusFinance {
  ...
  generateVirtualCard(
    ...
    const now = Date.now();
    const mat = offlineMaterialFromSeed(`janus:card:${identity.handle}:${now}`, 32);
    const cardNumber = this.generateCardNumber(mat);
    const cvv = (100 + Math.floor(unit(mat, 2) * 900)).toString();
    const expiry = new Date(now + 3600000); // 1 hora

    const card: VirtualCard = {
      id: `card_${Date.now()}_${deriveHexId(mat, "card", 0, 6)}`,
      number: cardNumber,
      cvv,
  ...
  private generateCardNumber(material: Uint8Array): string {
    const prefix = unit(material, 0) < 0.5 ? "4" : "5";
    let number = prefix;
    for (let i = 1; i < 16; i++) {
      number += Math.floor(unit(material, i) * 10).toString();
    }
    return number;
  }
  ...
  private addStatementLine(
    ...
    const mat = offlineMaterialFromSeed(`janus:stmt:${userHandle}:${Date.now()}`, 16);
    const line: StatementLine = {
      id: `stmt_${Date.now()}_${deriveHexId(mat, "stmt", 0, 6)}`,
      timestamp: Date.now(),
      ...data,
