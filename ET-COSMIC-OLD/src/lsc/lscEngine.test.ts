...
describe('lscEngine', () => {
  ...
  describe('modalCoherence', () => {
    ...
    it('coerência máxima quando todas as fases são iguais', () => {
      // Conforme Eq. 3, C_ε = |Σ |a_k| e^{iφ_k}| / Σ |a_k|
      // Quando todas as fases são iguais (ex: 0), C_ε = Σ|a_k| / Σ|a_k| = 1.0
    ...
    it('coerência com amplitude única', () => {
      // Qualquer amplitude única com qualquer fase tem C_ε = 1.0
      expect(modalCoherence([5], [0])).toBeCloseTo(1.0, 10);
      expect(modalCoherence([5], [Math.PI / 4])).toBeCloseTo(1.0, 10);
    });

    it('modos com amplitude zero são ignorados no cálculo de coerência', () => {
      // Apenas um modo ativo de 3
      const result = modalCoherence([1, 0, 0], [0, 0, 0]);
      expect(result).toBeCloseTo(1.0, 10);
    });

    it('fases diferentes com amplitudes diferentes resultam em coerência parcial', () => {
      // amplitudes: 3 e 4, fases: 0 e π/2
      // Σ|ak| = 7
      // Σ ak e^{iφk} = 3·e^{i0} + 4·e^{iπ/2} = 3 + 4i
      // módulo = 5
      // C_ε = 5 / 7 ≈ 0.7142857
      const result = modalCoherence([3, 4], [0, Math.PI / 2]);
