//! W1 — STM32F103 USART1 @ 0x40013800 + SPI2 @ 0x40003800 (dual page).
//! SPI1 @ 0x40013000 shares 4K page with USART1 — dual wedge uses SPI2.
...
#[test]
fn stm32_dual_mmio_yields_usart_and_spi_blocks() {
    let text = fs::read_to_string(pilot_dir().join("mmio_usart_spi.json")).unwrap();
    ...
    assert_eq!(
        spec.blocks.len(),
        2,
        "expected USART+SPI2 pages, blocks={:?}",
        spec.blocks
            .iter()
...
#[test]
fn stm32_dual_design_prefers_st_uart_and_spi() {
    let text = fs::read_to_string(pilot_dir().join("mmio_usart_spi.json")).unwrap();
    let accesses: Vec<MmioAccess> = serde_json::from_str(&text).unwrap();
    let (mut spec, _) = generate_spec_with_evidence(&accesses, "pilot_stm32/fw.bin");
    classify_by_page(&mut spec);
