  ...
  public async connectEternalNode() {
    ...
    setInterval(() => {
      if (this.eternalNodeConnected) {
        const mat = offlineMaterialFromSeed(`eternal:${this.sensorTick++}`, 8);
        this.realMetrics.rfVoltage = 1.2 + unit(mat, 0) * 0.5;
        this.realMetrics.thermalCurrent = 15 + unit(mat, 1) * 10;
